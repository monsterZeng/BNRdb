...
class DB():
    ...
    def insert(self, data, table_name,excel_keys):
        ...
        for i in range(0, len(data)):
            print(table_name, i + 1, "total nums = ", len(data))
            query = ""
            ...
            Nitrate_Removal_Rate   = str(data[excel_keys[8]][i]).replace("'","\\'")
            Denitrification_Rate   = str(data[excel_keys[9]][i]).replace("'","\\'")
            Major_Findings         = str(data[excel_keys[10]][i]).replace("'","\\'")
            Authors                = str(data[excel_keys[11]][i]).replace("'","\\'")
            Title                  = str(data[excel_keys[12]][i]).replace("'","\\'")
            Pubmed                 = str(data[excel_keys[13]][i]).replace("'","\\'")
            Link                   = str(data[excel_keys[14]][i]).replace("'","\\'")
            Abstract               = str(data[excel_keys[15]][i]).replace("'","\\'")
            
            query = """INSERT INTO %s (Influent_Source_Type,
...
if __name__ == "__main__":
    ...
    table_name = ["denitrifications_app_freshwatersystem",
                  ...
                  "denitrifications_app_sulphurremoval"]
    sheet_name_list = ["Freshwater systems", 
                       ...
                       "Sulphur removal"]
    #for i in range(4, len(sheet_name_list)):
    data = pd.read_excel("microbialdenitrification_systems_table.xlsx", sheet_name = sheet_name_list[-1])      
    mydb.insert(data, table_name[-1], data.keys())
    #print(data.keys())
